...
`make mojo-smoke` builds the Mojo interop image for the explicit Linux
`x86-64-v3` baseline. The build rejects EVEX, ZMM, or mask-register machine
code so GitHub runner features cannot leak AVX-512 into the release. The smoke
then exercises version and help output, configuration validation, and a live
server readiness check. Container publication repeats those runtime gates
against the immutable published Mojo digest.
